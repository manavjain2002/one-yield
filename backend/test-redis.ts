...
async function testConnection(urlOrOpts: any, label: string) {
...
async function run() {
  ...
  if (process.env.REDIS_PUBLIC_URL) {
    await testConnection(process.env.REDIS_PUBLIC_URL, 'REDIS_PUBLIC_URL');
  }
  if (process.env.REDIS_URL) {
    await testConnection(process.env.REDIS_URL, 'REDIS_URL');
  }
