...
function mapTx(t: ApiTx): TxHistory {
  ...
  const ty = ['borrow', 'repay', 'deposit', 'withdraw', 'transfer', 'create_pool', 'deploy_funds', 'activate', 'pause'].includes(t.type)
...
export function useManagerTxHistory() {
  return useQuery({
    queryKey: ['manager-tx-history'],
